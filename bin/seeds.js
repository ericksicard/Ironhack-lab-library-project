...
const DB_TITLE = 'ironhack-lab-library-project';
...
// createAuthors is now an array of promises that will create the authors inside the DB
const createAuthors = books.map(oneBook => {
  return Author.create(oneBook.author)
          .then(author => author.name )
          .catch( error => {throw new Error(`Impossible to add the author. ${error}`)} )
});

// findAuthors is now an array of promises that will update the books in the DB 
let findAuthors = Promise.all( createAuthors ) // here we wait till all the authors have been created in the DB
    // then we map through the books array to check if the books' authors are in the Author collection   
    .then( () => books.map( book => {          // we return an array of promises that will update the author information of every book in the array
      return Author.findOne({ name: book.author.name, lastName: book.author.lastName })
              .then( author => {
                if( !author ) throw new Error(`unknown author ${book.author.name} ${book.author.lastName}`)
...
.then( findAuthors => {
  ...
  .then( books => {
    return books.map( book => Book.create( book ) )
  })
})
.then( saveBooks => {
  Promise.all( saveBooks )
